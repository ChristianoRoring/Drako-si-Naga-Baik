...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadScene(pageIndex) {
    ...
    // 6. Ucapkan Narasi Cerita secara otomatis
    setTimeout(() => {
  ...
  function setupSyllableDragAndDrop(card, blockIdx) {
    ...
    card.addEventListener("pointermove", (e) => {
      ...
      if (!isDragging && dist > 10) {
        ...
        ghost.style.top = (e.clientY - initialRect.height / 2) + "px";
      ...
      if (isDragging && ghost) {
        ...
        checkSlotHover(e.clientX, e.clientY, blockIdx);
      }
    });
    ...
    card.addEventListener("pointerup", (e) => {
      ...
      if (isDragging) {
        if (ghost) {
          handleSyllableDrop(card, e.clientX, e.clientY, blockIdx);
          ghost.remove();
          ghost = null;
        }
      } else {
  ...
  function checkBlankHover(x, y, blockIdx) {
    const blank = document.getElementById(`blank-${blockIdx + 1}`);
    if (!blank) return;
    ...
    // Toleransi visual blank cukup besar (80px)
    if (dist < 80) {
  ...
  function setupBlankDropZones() {
    ...
    blanks.forEach(blank => {
      blank.addEventListener("click", () => {
        // Tidak menempatkan jawaban saat diklik, hanya bacakan kata jika rumpang sudah terisi
        const blankIdx = parseInt(blank.dataset.index) - 1;
        const bState = gameState.blanksState[blankIdx];
        if (bState.isPlacedInBlank) {
          audio.speakWord(bState.word);
        }
      });
  ...
  function setupCompleteCardDragAndDrop(cardEl, blockIdx) {
    ...
    cardEl.addEventListener("pointermove", (e) => {
      ...
      if (isDragging && ghost) {
        ghost.style.left = (e.clientX - ghost.offsetWidth / 2) + "px";
        ghost.style.top = (e.clientY - ghost.offsetHeight / 2) + "px";
        checkBlankHover(e.clientX, e.clientY, blockIdx);
      }
    });
    ...
    cardEl.addEventListener("pointerup", (e) => {
      ...
      const blank = document.getElementById(`blank-${blockIdx + 1}`);
      if (blank) blank.classList.remove("dragover");

      if (isDragging) {
        if (ghost) {
          handleCompleteCardDrop(cardEl, e.clientX, e.clientY, blockIdx);
          ghost.remove();
          ghost = null;
        }
      } else {
  ...
  function handleCompleteCardDrop(cardEl, dropX, dropY, blockIdx) {
    ...
    if (dist < 80) {
      snapWordToBlank(blockIdx);
    } else {
      audio.playClick();
    }
  }

  function snapWordToBlank(blockIdx) {
    const bState = gameState.blanksState[blockIdx];
    ...
    const blank = document.getElementById(`blank-${blockIdx + 1}`);
    if (blank) {
      blank.className = "word-drop-blank filled";
